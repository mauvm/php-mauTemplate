...
class mauTemplate
{
	...
	{
		if ( ! is_bool( $toScreen ) )
			throw new Exception( "Expecting parameter 1 to be a boolean" );
	...
	public function setIncludeBlockPattern( $pattern )
	{
		...
		if ( @preg_match( '/' . $pattern . '/', '' ) === false )
			throw new Exception( "Invalid pattern '/" . addslashes( $pattern ) . "/'" );
	...
	public function setBlockPattern( $blockStart, $blockEnd )
	{
		...
		if ( @preg_match( '/' . $blockStart . '/', '' ) === false )
			throw new Exception( "Invalid pattern '/" . addslashes( $blockStart ) . "/'" );
		if ( @preg_match( '/' . $blockEnd . '/', '' ) === false )
			throw new Exception( "Invalid pattern '/" . addslashes( $blockEnd ) . "/'" );
		...
		foreach ( $indexParts as $key => $part )
		{
			...
			if ( strpos( $part, ':' ) )
			{
				...
				if ( ! ctype_digit( $indexKey ) )
					throw new Exception( "Invalid index key '" . $indexKey . "' (must be numeric)" );
			}
