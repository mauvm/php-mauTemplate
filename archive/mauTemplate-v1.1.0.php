...
class mauTemplate
{
	...
	// 1. Code beautifying
	// 2. Block prefix/surfix
	// 3. Get include block name (length 2-32)
	 ...
	 * @return
	 */
	function assignInclude( $key, $file = '', $index = 0 )
	{
		if( is_string( $key ) ) $key = array( $key => & $file );

		if( is_array( $key ) )
		{
			if( ! $index || ! ctype_digit( "$index" ) ) $index = 0;

			// Loop and assign includes
			foreach( $key as $var => & $file )
			{
				// Verify index key
				if( $var != '__GLOBALS' && $var[0] != '/' && $var[0] != '+' )
				{
					// Assign global to given index (or the next one available)
					while( isset( $this->index['+' . $var][$index] ) ){ ++$index; }

					$this->index['+' . $var][$index] = (string) $file;
				}
			}
		}
	}
