...
class mauTemplate
{
	...
	public function setIndex( $mIndex, $bStrict = false, $bReturnArray = false, $bAppendBlock = false )
	{
		...
		{
			// Set index
			$iLastKey = end( $aKeys = array_keys( $aParts ) ); // Bugfix for reference error in PHP 5.2
			foreach( $aParts as $iKey => &$aPart )
			{
				...
				if( ! ctype_digit( "{$aPart[1]}" ) )
				{
					...
					if( $aPart[1] == 'first' )
						$aPart[1] = current( $aKeys = array_keys( $this->aIndex ) ); // Bugfix for reference error in PHP 5.2
					elseif ( $aPart[1] == 'last' || $aPart[1] === null )
						$aPart[1] = end( $aKeys = array_keys( $this->aIndex ) ); // Bugfix for reference error in PHP 5.2
				}
	...
	public function restoreIndex( $iHistory = 1, $bReturnArray = false )
	{
		...
		while ( $iHistory-- && end( $this->aIndexes ) !== null )
	 ...
	 * @return string or array
	 */
	public function newBlock( $mIndex = '.', $mData = null, $bReturnArray = false )
	{
		$mIndex = $this->setIndex( $mIndex, false, $bReturnArray, true );
		...
		if ( $mData )
		{
			...
			else
			{
				...
				$aLastPart = end( $this->aIndexParts );
				$this->assign( $aLastPart[0], $mData );
			}
		}
	 ...
	 * @return
	 */
	public function assign( $mKey, $sValue = '' )
	{
		if( is_string( $mKey ) )
	 ...
	 * @return
	 */
	public function assignGlobal( $mKey, $sValue = '' )
	{
		if( is_string( $mKey ) )
			$mKey = array( $mKey => &$sValue );
		if( is_array( $mKey ) )
	 ...
	 * @return
	 */
	public function assignInclude( $mKey, $sFile = '', $iIndex = null )
	{
		if( is_string( $mKey ) )
		...
		if( is_array( $mKey ) )
		{
			...
			foreach( $mKey as $sKey => &$sFile )
			{
				...
				if( $sKey != '__GLOBALS' && $sKey[0] != '/' && $sKey[0] != '+' )
				{
					...
					if( $iIndex !== null && ctype_digit( "{$iIndex}" ) )
					{
	...
	protected function _parse( $sTemplate, array & $aData = null )
	{
		...
		if( preg_match_all( $this->sVariablePattern, $sTemplate, $aVariables, PREG_SET_ORDER ) )
		{
			...
			foreach( $aVariables as & $aVariable )
			{
				...
				if( $aVariable[0][0] == "\r" || $aVariable[0][0] == "\n" )
				{
				...
				$sTemplate = substr_replace( $sTemplate, str_replace( $this->aEscapeChars[0], $this->aEscapeChars[1], $_sReplaceData ), strpos( $sTemplate, $_sReplaceSearch ), strlen( $_sReplaceSearch ) );
			}
		}
