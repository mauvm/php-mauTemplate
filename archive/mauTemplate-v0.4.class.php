 ...
 * TODO:
 *   - Add caching
 *   - Fix clearData parent (to remove the NOTE)
 *   - Add prevBlock & nextBlock functions
 * 
 * @category   HTML
 * @package   
...
class mauTemplate
{
	 ...
	 * 
	 * NOTE: Current index is used on empty (instead of root index)
	 * 
	 * @param string $index
	 * @return int
	 */
	public function getBlockCount( $index = '' )
	{
		...
		if ( empty( $index ) )
			...
			$index = $this->getCleanIndex( $index ); // Clean index
	 ...
	 * 
	 * NOTE: If current index equals index to remove, afterwards index will be set to its parent
	 * 
	 * @param $index
	 * @return
	 */
	public function clearData( $index = '' )
	{
		...
		if ( $index || $index == './' )
		{
			...
			// Restore index
			if ( $currIndex == $index )
				$currIndex .= '/..'; // Goto parent if current block is cleared
				...
				if ( isset( $data[$_blockName] ) && is_array( $data[$_blockName] ) )
				{
					...
					foreach ( array_keys( $data[$_blockName] ) as $_blockKey )
					{
						if ( is_array( $data[$_blockName][$_blockKey] ) )
							$_replaceData .= $this->parse( $blocks[$key][2], $data[$_blockName][$_blockKey] );
						else
							$_replaceData .= $this->parse( $blocks[$key][2] ); // No data to pass on
					}
				}
